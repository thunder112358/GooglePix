...
def init_block_matching(ref_img, options, params):
    ...
    check_gpu()
    th_ref_img = torch.as_tensor(ref_img, dtype=DEFAULT_TORCH_FLOAT_TYPE, device="cuda:0")
...
def align_image_block_matching(img, referencePyramid, options, params, debug=False):
    ...
    check_gpu()
    th_img = torch.as_tensor(img, dtype=DEFAULT_TORCH_FLOAT_TYPE, device="cuda:0")
